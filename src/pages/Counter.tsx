...
export default function Counter({ triggerCelebration }: CounterProps) {
  const { challenge, user, addReps } = useOutletContext<{
    challenge: Challenge;
    user: User;
    users: User[];
  ...
  const inc = (n: number) => {
    ...
    if (newCount >= challenge.goalCounterUser) {
      triggerCelebration("Awesome! Keep it up!", 3);
